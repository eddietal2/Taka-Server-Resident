...
} from '../src/lib/providers/africastalking';
...
function acceptedResponse(): HttpResponseLike {
  return jsonResponse({
    SMSMessageData: {
...
function providerWith(fetchImpl: FetchLike, senderId?: string): AfricasTalkingOtpProvider {
...
describe('AfricasTalkingOtpProvider', () => {
  ...
  it('includes from only when a sender ID is configured', async () => {
    ...
    const withInit = withSender.mock.calls[0][1] as FetchInit;
    expect(new URLSearchParams(withInit.body).get('from')).toBe('TAKA');

    const withoutSender = vi.fn<FetchLike>().mockResolvedValue(acceptedResponse());
    await providerWith(withoutSender).send(PHONE, '123456');

    const withoutInit = withoutSender.mock.calls[0][1] as FetchInit;
    expect(new URLSearchParams(withoutInit.body).has('from')).toBe(false);
  });
  ...
  it('rejects with a 503 when no recipients are returned', async () => {
    const fetchImpl = vi
      .fn<FetchLike>()
      .mockResolvedValue(jsonResponse({ SMSMessageData: { Message: 'Sent to 0/0', Recipients: [] } }));
